...
# Generic "winning state" signals: a placed order, or a PortSwigger lab solved banner
# (the storefront renders class "is-solved" / "Congratulations, you solved the lab!").
SOLVED = ("status == 200 and ('is-solved' in text or 'congratulations' in text.lower() "
          "or 'solved the lab' in text.lower() or 'order placed' in text.lower() "
          "or 'order confirmed' in text.lower())")
...
def purchase_flow(registry: EndpointRegistry, identities) -> list[TestCase]:
    ...
    for ep in registry:
        ...
        steps.append(TestStep(step=next(n), description="Checkout (place order)",
                              ...
                              success_condition=("status in (200, 302) and ("
                                                 "'order placed' in text.lower() or "
                                                 "'order confirmed' in text.lower() or "
                                                 "'is-solved' in text)")))
...
def cart_balancing(registry: EndpointRegistry, identities) -> list[TestCase]:
    ...
    for ep in registry:
        ...
        steps.append(TestStep(step=next(n), description="Checkout (place order)",
                              method="POST", path=checkout_path, identity=attacker,
                              form={"csrf": "{ck_csrf}"}, follow_redirects=True,
                              success_condition=("status in (200, 302) and ('order placed' in text.lower() "
                                                 "or 'order confirmed' in text.lower() or 'is-solved' in text)")))
        steps.append(TestStep(step=next(n), description="Confirm order / lab-solved state",
